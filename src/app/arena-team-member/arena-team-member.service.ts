...
export class ArenaTeamMemberService {
  ...
  private loadTeamDetail(): void {
    this.teamDetails$ = this.http
      .get<ArenaTeam[]>(
        API_URL + "/characters/arena_team/id/" + this.arenaTeamId
      )
      .pipe(
        map((data: ArenaTeam[]) => {
  ...
  private loadTeamMemberDetail(): void {
    this.members$ = this.http
      .get<ArenaTeamMember[]>(
        API_URL + "/characters/arena_team_member/" + this.arenaTeamId
      )
      .pipe(
        map((data: ArenaTeamMember[]) => {
          data.forEach((value: ArenaTeamMember, idx: number) => {
            ...
            data[idx].weekNeeded =
              value.weekGames > 9 ? 0 : 10 - value.weekGames;
